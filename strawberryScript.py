...
# # Define the range of images you want to process
# start_index = 0  # Starting index of the images
# end_index = 100  # Ending index of the images

def extract_and_save_results(start_index, end_index, im_ids_subset, data_path):
    # Open CSV file for writing
    csv_file_path = "results.csv"
    csv_file = open(csv_file_path, "w", newline="")
    csv_writer = csv.writer(csv_file)
    csv_writer.writerow(["Image Name", "Approximated Count", "True Count"])  # Write header


    # Loop through the images in the specified range
    im_ids_subset = im_ids[start_index:end_index]  # Assuming im_ids is a list of image IDs
    for im_id in im_ids_subset:
        # Your existing code to process each image
        # (...)
        ...
        for im_id in pbar:
            ...
            # Compute approximate count and true count
            # For example, assuming pred_cnt is the approximate count and gt_cnt is the true count
            pred_cnt = output.sum().item()
            gt_cnt = dots.shape[0]
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Extract results and save to CSV")
    parser.add_argument("data_path", type=str, help="Path to the data directory")
    parser.add_argument("--start_index", type=int, default=1, help="Starting index of the images")
    parser.add_argument("--end_index", type=int, default=169, help="Ending index of the images")
    parser.add_argument("output", type=str, help="Output tensor")
    parser.add_argument("dots", type=str, help="True count")
    args = parser.parse_args()

    data_path = args.data_path
    start_index = args.start_index
    end_index = args.end_index
    output = args.output  # Assuming output tensor is provided as a string
    dots = args.dots  # Assuming true counts are provided as a string
    
    # Convert output and dots to appropriate data types if necessary
    
    
    # Generate the list of image IDs
    im_ids_subset = ['image{}.jpg'.format(i) for i in range(start_index, end_index + 1)]
    
    # Call the function to extract and save results
    extract_and_save_results(start_index, end_index, im_ids_subset, data_path)
